 ...
 *
 * The counts span 301,784 to 0, so a linear bar would render every rung but
 * the first as nothing. The widths are logarithmic and say so — a scale that
 * flatters the small numbers without admitting it would be the same dishonesty
 * this whole page is about.
 */
const width = (n: number | null): string => {
  if (n === null) return "0%";
  if (n <= 0) return "1.5%";
  const max = Math.log10(301_784 + 1);
