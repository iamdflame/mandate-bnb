...
import { safeFetch, RefusedUrl, whyUnsafe, whyUnsafeHost } from "@/lib/net/safe-fetch";
...
export interface ProbeResult {
  ...
  fingerprint?: string;
  /** True when we would not call the endpoint at all: plain http, or an address on a private network. */
  refused?: boolean;
  error: string | null;
  /**
  ...
  at: string;
  /**
   * The exchange, for the reader who wants to check the grade: what was asked,
   * the status, the start of each answer, and the hash of all of it. The
   * excerpt is short because every page loads this reading for hundreds of
   * agents; the hash is what makes it checkable.
   */
 ...
 */
async function tryMcp(url: string, transcript: Step[]): Promise<{ tools: ProbeTool[]; status: number; ms: number } | null> {
  ...
  if (!listed) return { tools: [], status: res.status, ms };
  ...
  return { tools, status: res.status, ms };
}
...
async function tryA2a(url: string, transcript: Step[]): Promise<{ tools: ProbeTool[]; status: number; ms: number } | null> {
  ...
  if (rpc) {
    ...
    if (isJsonRpc(body)) return { tools: skills, status: rpc.status, ms: Date.now() - t0 };
  }

  // A card alone still proves an A2A agent is published there.
  if (skills.length && cardStatus === 200) return { tools: skills, status: 200, ms: cardMs ?? Date.now() - t0 };
  return null;
}

/** The plain call, which is still how an x402 seller quotes a price. */
async function tryHttp(url: string, transcript: Step[]): Promise<{ status: number; x402: boolean; ms: number } | null> {
  const t0 = Date.now();
  const res = await safeFetch(url, {
  ...
  return { status: res.status, x402: quoted, ms: Date.now() - t0 };
}
...
export async function probe(tokenId: string, endpoint: string | null): Promise<ProbeResult> {
  ...
  // Checked before anything is sent, the name resolved as well as read.
  const unsafe = whyUnsafe(endpoint) ?? (await whyUnsafeHost(new URL(endpoint).hostname));
  if (unsafe) {
    return { ...base, answered: false, status: null, latencyMs: null, protocol: null, refused: true, error: unsafe };
  }
  // A refusal mid-way (a redirect somewhere we will not follow) is a finding in itself, so it is not swallowed with the other failures.
  const unlessRefused = (e: unknown): null => {
    if (e instanceof RefusedUrl) throw e;
    return null;
  };
  ...
  try {
    /*
      The plain GET decides how hard to try. A host that did not resolve,
      refused the connection or failed TLS will fail every handshake the same
      way, so nothing more is sent: a dead agent costs one attempt, not six.
      A GET that timed out may be an MCP server holding the GET open as an
      event stream, so MCP is still tried; A2A is only tried after a reply.
    */
    let getFailure: unknown = null;
    const plain = await tryHttp(endpoint, transcript).catch((e: unknown) => {
      unlessRefused(e);
      getFailure = e;
      return null;
    });
    const timedOut = getFailure !== null && /timeout|abort|no answer/i.test(String(getFailure));
    if (!plain && !timedOut) {
      return { ...base, answered: false, status: null, latencyMs: Date.now() - started, protocol: null, error: classifyFailure(getFailure), transcript };
    }
    ...
    if (plain?.x402) {
      return {
        ...
        answered: true,
        status: plain.status,
        // The time of the request that proved it, not of the whole probe.
        latencyMs: plain.ms,
        protocol: "x402",
        error: null,
    ...
    const mcp = await tryMcp(endpoint, transcript).catch(unlessRefused);
    if (mcp) {
      return {
        ...base,
        answered: true,
    ...
    const a2a = plain ? await tryA2a(endpoint, transcript).catch(unlessRefused) : null;
    if (a2a) {
      return {
        ...base,
        answered: true,
    ...
    */
    ...
    return {
      ...
      protocol: null,
      ...(e instanceof RefusedUrl ? { refused: true } : {}),
...
export async function probeAll(
  ...
  concurrency = 6,
  /** No new agent is started after this time; the ones left keep their old reading and go first next time. */
