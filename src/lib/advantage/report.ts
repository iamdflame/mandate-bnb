 ...
 *
 * The losses and the mixed results stay exactly as prominent as the wins,
 * because a report where the agent wins everything is a report nobody should
 * believe. How many of each there are is counted here, never written down.
 */
...
/**
 * The locked specification is hashed on chain, so its text cannot be edited,
 * and it was written with dashes. Wherever it is shown the dashes read as
 * commas, as the report has always printed them. The hash is of the stored
 * text, untouched, so anyone checking it still hashes the original.
 */
export const plain = (s: string): string => s.replace(/\s*—\s*/g, ", ");
...
export function readAdvantage(): AdvantageReading | null {
  ...
  const tasks: TaskReport[] = lock.specification.tasks.map((spec) => {
    ...
    return {
      ...
      humanArm: plain(spec.humanArm),
      agentArm: plain(spec.agentArm),
      metric: plain(spec.metric),
      verdict,
      line,
      result,
    };
