...
export function whyUnsafe(raw: string): string | null {
  ...
  if (PRIVATE_V4.some((re) => re.test(host))) return "that address is on a private or reserved network";

  // IPv6: loopback, unique local (fc00::/7), and link local (fe80::/10).
  if (host === "::1" || host === "::" || /^f[cd]/i.test(host) || /^fe[89ab]/i.test(host)) {
    return "that address is on a private or reserved network";
  }
  // An IPv4 address mapped into v6 still points where it points.
